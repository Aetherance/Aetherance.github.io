---
title: "西邮Linux兴趣小组2022纳新题解"
description: "感谢 Zhilu 重新录入题目原件。好人一生平安。"
pubDate: 2024-10-27
...
又因为`2^10 = 1024`对应十进制的位 所以`2^10000`对应二进制的`3011`位
...
> sizeof()与strlen()的不同：  
> sizeof()`所计算出的是变量的大小 以字节为单位`strlen()`所计算出的是字符串的长度 遇到'\0'时结束 不包含'\0'`strcmp()`函数用于比较两个以’\\0’结束的字符串 如果相同则返回 ***0***
...
`sizeof(p0)`求出的是字符串包含`\0`的大小 `sizeof(p1)`求出的指针的大小 `sizeof(*p2)`求出的是一个字符的大小 `strlen(p0)`求出的是p0的长度 `strlen(p1)`求出的是字符串p1的长度。
...
```c
400000100 500000101600000110
700000111700000111700000111
a00000111 (7)b00000101 (5)c00000001 (1)
a(<<=3)00111000 (56)b(>>=3)00000000 (0)c(=~c)11111110 (254)

a00111000e 10000001
c11111110
a^c0000000011000110e 10000100 (4)
d0000011000110000 (1584)
(char)d00110000 (48)
```
